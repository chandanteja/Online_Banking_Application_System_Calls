    ...
    //  printf("Inside verification loop\n");
      ...
      switch(op_choice)
      {
        ...
        case 3:   //check balance
              ...
              break;
        case 4:     //View Details i.e print all the users transactions
              break;
...
else if(acc_login_choice==2)
{
      ...
      char username_password_prompt[250]="\n==================Enter Username and Password for Joint Account Login.Press Enter to continue==================\n";
      char username_prompt[100]="Username: ";
      char password_prompt[100]="Password: ";
      char response_ch;
      write(connfd_customer,username_password_prompt,sizeof(username_password_prompt));
      ...
      if(response_ch=='\n')
      {
        ...
        struct Joint_Account credentials_cust;
        int n_cust;  //keep track of how many characters read has read from file
        ...
        lock_login_cust.l_len=0; //entire file is locked from start
        lock_login_cust.l_pid=getpid();
        fd_lgn_cust=open("joint_accounts.txt",O_RDONLY);
        perror("open");
          ...
          //  printf("Inside verification loop\n");
            ...
            switch(op_choice)
            {
              case 1:   //Deposit
                          ...
                          printf("Collecting data for deposit Joint account\n");
                          int acc_no;
                          double amnt;
                          char username[ARRAY_SIZE];
                          read(connfd_customer,username,sizeof(username));
                          printf("username for joint acc: %s\n",username);
                          read(connfd_customer,&acc_no,sizeof(acc_no));
                          acc_no=ntohl(acc_no);
                          printf("acc_no: %d\n",acc_no);
                          read(connfd_customer,&amnt,sizeof(amnt));
                          printf("amount= %lf\n",amnt);
                          int dep_ret=depositJoint(acc_no,amnt);
                          if(dep_ret==1)
                          {
                              char success[150]="Amount deposited successfully in joint account\n";
              ...
              case 2:     //withdraw
                        ...
                        //        double amnt;
                                ...
                                read(connfd_customer,&amnt,sizeof(amnt));
                                printf("amount= %lf\n",amnt);
                                int with_ret=withdrawJoint(acc_no,amnt);
                                if(with_ret==1)
                                {
                                    char success[150]="Amount debited successfully from joint account\n";
              ...
              case 3:   //check balance
                      ...
                      //        int acc_no;
                            //  double amnt;
                              ...
                              //read(connfd_customer,&amnt,sizeof(amnt));
                              //printf("amount= %lf\n",amnt);
                              int enq_ret=checkBalanceJoint(acc_no);
                              if(enq_ret==1)    //successfully goot balance details
                              {
                                  char success[150]="Balance Retrieved Successfully\n";
